...
export const ObjectListener = {
    get(): OBJECT_LISTENER_TYPE {
        let r = {};
        Object.keys(ObjectListener_).forEach(k => { (r as any)[k] = (ObjectListener_ as any)[k]; });
        return r as OBJECT_LISTENER_TYPE;
    },

    patch(l: OBJECT_LISTENER_TYPE) {
        Object.keys(l).forEach(k => {
            if((l as any)[k] instanceof Function) {
                (l as any)["_"+k] = (l as any)[k];
                (l as any)[k] = function(...args: any): any { return (l as any)["_"+k](...args); }
            }
        });
        return l;
    }
